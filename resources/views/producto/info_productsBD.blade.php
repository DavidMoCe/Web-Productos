    ...
    <x-app-layout>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                @if (!request()->routeIs('admin.dashboard'))
                    <a href="{{ route('products') }}" class="text-zinc-800 hover:underline">{{ __('Products') }}</a>
                @else
                    {{ __('Products') }}
                @endif
            </h2>
        ...
        @php
            ...
            $productoDiferente = $info_producto->first(function ($producto) use ($info_producto,$capacidad,$color,$mejor_precio,$mas_popular) {
                // Comprobar si el producto coincide con la capacidad y el color
                $matchCapacidad = $producto->capacidad == $capacidad;
                $matchColor = $producto->color == $color;
                ...
                if (isset($mejor_precio) && $mejor_precio == 'si') {
                    ...
                    $precioMasBajo = $info_producto->where('nombre', $producto->nombre)->min('precioD');
                    return $producto->precioD == $precioMasBajo;
                }
                if(isset($mas_popular) && $mas_popular == 'si'){
                    // Obtener el producto con la cantidad más baja
                    $cantidadMasBaja =$info_producto->where('nombre', $producto->nombre)->min('stock');
                    return $producto->stock == $cantidadMasBaja;
                }
            ...
            if ($productoDiferente) {
                ...
                $sku_title = $productoDiferente->nombre.' '.$productoDiferente->capacidad.' - '.$productoDiferente->color.($productoDiferente->libre ? ' - Libre' : '').($productoDiferente->bateria ? ' '.$productoDiferente->bateria : '').' '.$productoDiferente->estado;
                $titulo = $productoDiferente->nombre.' '.$productoDiferente->capacidad.' '.$productoDiferente->color.($productoDiferente->libre ? ' - Libre' : '');
                $nombre_producto= $productoDiferente->nombre;
                $precio = $productoDiferente->precioD;
                $stock = $productoDiferente->stock;
                $descripcion = $productoDiferente->descripcion;
            }
            
            //sacamos el nombre de la imagen
            $nombre= $productoDiferente->nombre;
            
            $patron = '/iphone \d+/i';
            if (preg_match($patron, $nombre, $matches)) {
                $nombre= $matches[0];
            }
            $nombreImagen= str_replace(' ','',$nombre.'-'.$productoDiferente->color);
            //la ponemos en minuscula
            $nombreImagen= strtolower($nombreImagen);

        @endphp
        <form id="carrito-form" action="{{ route('cart.add') }}" method="POST">
            @csrf <!-- Agrega este token CSRF para proteger tu formulario de ataques CSRF-->
            <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
                    <div class="lg:flex items-start justify-center py-12 2xl:px-20 sm:!px-8 px-4">
                        ...
                        <input name="titulo_imagen" value="{{ $nombreImagen }}" class="hidden peer" readonly />
                            ...
                            <div class="product-details">
                                <div id=titulo-botonCompra class="dark:text-white">
                                    ...
                                    <div class="actions mb-4 flex flex-row">
                                        ...
                                        <div class="relative mx-auto lg:!mx-0 md:mt-4">
                                            <button id="buyButton" type="submit" class="relative px-20 py-3 md:px-40 lg:px-16 text-xl md:text-2xl bg-slate-900 dark:bg-slate-50 dark:hover:bg-slate-200 text-white dark:!text-neutral-800 rounded motion-safe:transition-colors motion-safe:duration-300 motion-safe:ease-out">
                                                Comprar
                                            </button>
                                            <div id="loadingIcon" role="status" class="absolute inset-0 flex items-center justify-center  transition-opacity duration-200">
                                    ...
                                    <div id="lo_mejor" class="mb-6 lg:!mb-12">
                                        ...
                                        <ul class="grid list-none gap-2 grid-cols-2">
                                            <li data-qa="recommended_variants-0">
                                                @php
                                                    $producto_mejor = $info_producto->sortBy('precioD')->where('stock', '>', 0)->first();
                                                    $producto_mejor_titulo = $producto_mejor->nombre;
                                                    $producto_mejor_capacidad = $producto_mejor->capacidad;
                                                    $producto_mejor_color= $producto_mejor->color;
                                                    
                                                @endphp
                                                <a aria-current="page" href={{ url('/info_products?' . http_build_query(['producto' => $producto_mejor_titulo, 'capacidad' => $producto_mejor_capacidad, 'color' =>  $producto_mejor_color ,'mejor_precio' => 'si'])) }} :active="request()->routeIs('products')"
                                                    ...
                                                    <div class="w-full">
                                                        ...
                                                        <div class="text-static-brand-hi text-center">{{ number_format($producto_mejor->precioD, 2,",", ".") }}&nbsp;€</div>
                                                    </div>
                                                </a>
                                            </li>
                                            <li data-qa="recommended_variants-1">
                                                @php
                                                    $producto_popular = $info_producto->sortBy('stock')->where('stock', '>', 0)->first();
                                                    $producto_popular_titulo = $producto_popular->nombre;
                                                    $producto_popular_capacidad = $producto_popular->capacidad;
                                                    $producto_popular_color= $producto_popular->color;
                                                @endphp
                                                <a aria-current="page" href="{{ url('/info_products?' . http_build_query(['producto' => $producto_popular_titulo, 'capacidad' => $producto_popular_capacidad,'color' =>  $producto_popular_color ,'mas_popular' => 'si'])) }}"
                                                    ...
                                                    <div class="w-full">
                                                        ...
                                                        <div class="text-static-brand-hi text-center">{{ number_format($producto_popular->precioD, 2,",", ".") }}&nbsp;€</div>
                                                    </div>
                                                </a>
                                            </li>
                                        </ul>
                                        ...
                                        <ul id="ul-estado" class="grid list-none gap-2 grid-cols-3 mb-4">
                                            ...
                                            @php
                                                ...
                                                $productos_correcto = $info_producto->filter(function ($producto) use ($capacidad, $color) {
                                                    return ($producto->estado == "COR" || $producto->estado == "STA") &&
                                                        strpos($producto->capacidad, $capacidad) !== false &&
                                                        strpos($producto->color, $color) !== false;
                                                });
                                                //parte de MBU
                                                $productos_bueno = $info_producto->filter(function ($producto) use ($capacidad, $color) {
                                                    return ($producto->estado == "MBU" || $producto->estado == "BUE") &&
                                                        strpos($producto->capacidad, $capacidad) !== false &&
                                                        strpos($producto->color, $color) !== false;
                                                });
                                                //parte de IMP
                                                $productos_impecable = $info_producto->filter(function ($producto) use ($capacidad, $color) {
                                                    return ($producto->estado == "IMP") &&
                                                        strpos($producto->capacidad, $capacidad) !== false &&
                                                        strpos($producto->color, $color) !== false;
                                                });
                                                ...
                                                if($sku_title && preg_match('/NEW\s*BATTERY/i', $sku_title)){
                                                    // Seleccionar un producto con 'NEWBATTERY', seleccionar el primero que lo tenga
                                                    //Producto CORRECTO
                                                    $producto_COR = $productos_correcto->filter(function ($producto) {
                                                        return preg_match('/NEW\s*BATTERY/i', $producto->bateria);
                                                    })->first();
                                                    //Producto MuyBueno
                                                    $producto_MBU = $productos_bueno->filter(function ($producto) {
                                                        return preg_match('/NEW\s*BATTERY/i', $producto->bateria);
                                                    })->first();
                                                    //Producto Impecable
                                                    $producto_IMP = $productos_impecable->filter(function ($producto) {
                                                        return preg_match('/NEW\s*BATTERY/i', $producto->bateria);
                                                    })->first();
                                                }else {
                                                ...
                                                if ($producto_COR) {
                                                    $precio_producto_Correcto = number_format($producto_COR->precioD , 2, ",", ".");
                                                }
                                                // Si se encontró un producto seleccionado, calcular su precio correcto
                                                if ($producto_MBU) {
                                                    $precio_producto_MBU = number_format($producto_MBU->precioD , 2, ",", ".");
                                                }                                      
                                                // Si se encontró un producto seleccionado, calcular su precio correcto
                                                if ($producto_IMP) {
                                                    $precio_producto_IMP = number_format($producto_IMP->precioD , 2, ",", ".");
                                                }
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                @foreach ($info_producto as $producto)
                    var capacity = extractCapacity("{{ $producto->capacidad }}");
                    uniqueCapacities.add(capacity);
                @endforeach

                // Función para extraer los gigabytes del título del producto
                function extractCapacity(title) {
                    var matches = title.match(/^(\d+)\s*([MTG]B\b)$/i);

                    if (matches) {
                        var capacity;
                        if (matches[2].toUpperCase() === "GB") {
                            capacity = matches[1] + "GB";
                        } else if (matches[2].toUpperCase() === "MB") {
                            capacity = matches[1] + "MB";
                        }else if (matches[2].toUpperCase() === "TB") {
                            capacity = matches[1] + "TB";
                        } else {
                            // Si no es GB ni MB, asumimos GB por defecto
                            capacity = matches[1] + "GB";
                        }
                    ...
                    var capacity = matches ? matches[0] : "Sin capacidad";
                ...
                function extractColor(title) {
                    var matches = title.match(/\s*([a-zA-ZáéíóúñÑÁÉÍÓÚ\s]+)/);
                    if (matches) {
                        // Obtener el color encontrado
                        var color = matches[0].trim();

                        // Reemplazar valores específicos
                        if (color.toLowerCase() === "product") {
                            color = "Rojo";
                        } 
                        return color;
                    } else {
                        return "Sin color";
                    } // Para eliminar espacios en blanco al principio y al final, si los hubiera
                }
                ...
                @foreach ($info_producto as $producto)
                    var color = extractColor("{{ $producto->color }}");
                ...
                var tituloProducto ="<?php echo $nombre_producto; ?>" // Obtener el título del producto
            ...
            $(document).ready(function() {
                ...
                function manejarSolicitudAjax() {
                    ...
                    var tituloProducto = "<?php echo $nombre_producto; ?>";
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (!isNaN(precio)) {
                            ...
                            if(response.imput=="true"){
                                $('#battery-checkbox').prop('checked', true);
                            }else{
                                $('#battery-checkbox').prop('checked', false);
                            }
